...
function LoginForm() {
  ...
  const getErrorMessage = (error: string | null) => {
    ...
    switch (error) {
      ...
      default:
        return 'Sign-in failed. Please try again or use Demo Host access.';
    }
  };
    ...
    <div className="w-full max-w-md bg-[#0f111a]/90 backdrop-blur-2xl border border-white/10 rounded-3xl p-8 shadow-2xl relative z-10 text-white">
      ...
      <div className="flex flex-col items-center text-center mb-6">
        ...
        <h1 className="text-2xl font-black tracking-tight text-white flex items-center gap-2">
          Host Sign In <Sparkles className="w-4 h-4 text-amber-400" />
        </h1>
        <p className="text-xs text-zinc-400 mt-1 max-w-xs">
          Sign in to create & manage events, generate venue QR codes, and moderate guest video greetings.
        </p>
      </div>
        ...
        <form onSubmit={handleDemoSignIn} className="space-y-3.5">
          ...
          >
            {loadingDemo ? (
              ...
              <>
                <span>Enter Host Dashboard</span>
